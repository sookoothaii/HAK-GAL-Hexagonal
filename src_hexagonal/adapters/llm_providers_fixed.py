...
class GeminiProvider(LLMProvider):
    ...
    def __init__(self):
        ...
        # FIXED: Use gemini-1.5-flash directly (not flash-latest which is rate limited)
        self.models = [
            "gemini-1.5-flash",          # This one works! Use it first
            "gemini-1.5-pro",            # Fallback to pro if needed
...
class MultiLLMProvider(LLMProvider):
    ...
    def __init__(self, providers: Optional[List[LLMProvider]] = None):
        if providers is None:
            providers = [
                GeminiProvider(),     # Gemini first - now uses flash not flash-latest
        ...
        for i, provider in enumerate(self.providers):
            ...
            if provider.is_available():
                ...
                try:
                    ...
                    if response and not any(err in response.lower()[:100] for err in error_indicators):

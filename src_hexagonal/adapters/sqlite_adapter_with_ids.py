...
ENHANCED: Now includes ID generation for Facts
"""
...
class SQLiteFactRepository(FactRepository):
    ...
    Uses production database hexagonal_kb.db
    ENHANCED: Generates IDs for facts using ROWID
    """
    ...
    def find_by_query(self, query: str, limit: int = 10) -> List[Fact]:
        """Suche Facts mit LIKE Query - ENHANCED with IDs"""
        facts = []
        try:
            with self._connect() as conn:
                cursor = conn.execute(
                    'SELECT ROWID, statement FROM facts WHERE statement LIKE ? LIMIT ?',
                    (f'%{query}%', limit)
                )
                ...
                for row in cursor:
                    fact = Fact(
                        statement=row[1],
                        context={},
                        metadata={'id': row[0]},  # Add ROWID as ID
                        confidence=1.0
                    )
                    facts.append(fact)
        except Exception as e:
    ...
    def find_all(self, limit: int = 100) -> List[Fact]:
        """Hole alle Facts - ENHANCED with IDs"""
        facts = []
        try:
            with self._connect() as conn:
                cursor = conn.execute(
                    'SELECT ROWID, statement FROM facts ORDER BY ROWID LIMIT ?',
                    (limit,)
                )
    ...
    def find_page(self, offset: int, limit: int) -> List[Fact]:
        """Hole Facts mit OFFSET/LIMIT (Pagination) - ENHANCED with IDs"""
        facts: List[Fact] = []
        try:
            with self._connect() as conn:
                cursor = conn.execute(
                    'SELECT ROWID, statement FROM facts ORDER BY ROWID LIMIT ? OFFSET ?',
                    (limit, offset)
                )
                for row in cursor:
                    fact = Fact(

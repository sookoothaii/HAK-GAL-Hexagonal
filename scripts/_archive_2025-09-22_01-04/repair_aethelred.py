...
# -*- coding: utf-8 -*-
"""
FIX für Aethelred Engine Crash - BEIDE ENGINES SIND WICHTIG!
"""
...
def diagnose_and_fix_aethelred():
    """Diagnostiziere und repariere Aethelred Engine"""
    
    print("🔧 AETHELRED ENGINE REPAIR")
    print("=" * 60)
    
    base_path = Path(r"D:\MCP Mods\HAK_GAL_HEXAGONAL")
    aethelred_path = base_path / "src_hexagonal" / "infrastructure" / "engines" / "aethelred_fast.py"
    thesis_path = base_path / "src_hexagonal" / "infrastructure" / "engines" / "thesis_fast.py"
    
    # 1. Prüfe ob Dateien existieren
    print("\n📁 Checking engine files...")
    if aethelred_path.exists():
        print(f"   ✅ Aethelred found: {aethelred_path.name}")
    else:
        print(f"   ❌ Aethelred MISSING!")
        
    if thesis_path.exists():
        print(f"   ✅ Thesis found: {thesis_path.name}")
    else:
        print(f"   ❌ Thesis MISSING!")
    
    # 2. Test Dependencies
    print("\n📦 Testing dependencies...")
    python_exe = base_path / ".venv_hexa" / "Scripts" / "python.exe"
    
    # Test ob Python funktioniert
    try:
        ...
        print(f"   ✅ Python works: {result.stdout.strip()}")
    except Exception as e:
        print(f"   ❌ Python error: {e}")
    
    # Test kritische Imports
    critical_imports = ["sqlite3", "requests", "argparse", "time", "random"]
    for module in critical_imports:
        try:
            ...
            if "OK" in result.stdout:
                ...
                print(f"   ❌ {module} FAILED: {result.stderr}")
        except Exception as e:
            print(f"   ❌ {module} error: {e}")
    
    # 3. Fix Aethelred Code
    print("\n🔧 Fixing Aethelred engine code...")
    
    fixed_aethelred = '''#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""
...
'''
    ...
    # Backup original
    import shutil
    backup_path = aethelred_path.with_suffix('.py.backup_crash')
    shutil.copy(aethelred_path, backup_path)
    print(f"   ✅ Backup created: {backup_path.name}")
    
    # Write fixed version
    aethelred_path.write_text(fixed_aethelred, encoding='utf-8')
    print(f"   ✅ Fixed version written")
    
    # 4. Test the fixed engine
    print("\n🧪 Testing fixed Aethelred...")
    try:
        result = subprocess.run(
            [str(python_exe), str(aethelred_path), "-d", "0.01", "-p", "5002"],
            capture_output=True,
            text=True,
            timeout=10
        )
        if result.returncode == 0:
            print("   ✅ Aethelred test SUCCESSFUL!")
    ...
    print("\nBOTH ENGINES ARE ESSENTIAL:")
    print("- AETHELRED: Generates NEW external facts from LLM")
    print("- THESIS: Finds patterns and relationships in EXISTING facts")
    print("\n⚠️ RESTART the Governor from the Frontend!")
    print("The engines should now work properly.")

if __name__ == "__main__":
    diagnose_and_fix_aethelred()

...
class HAKGALDiagnosticsIntegration:
    ...
    def test_hakgal_connection(self):
        ...
        try:
            ...
            if response.status_code == 200:
                logger.info("✅ HAK/GAL API connection successful")
                return True
            else:
                logger.error(f"❌ HAK/GAL API connection failed: {response.status_code}")
                return False

        except Exception as e:
            logger.error(f"❌ HAK/GAL API connection error: {e}")
            return False
    ...
    def log_agent_metric_to_kb(self, agent_name, metric_type, value, context=""):
        ...
        try:
            ...
            if response.status_code == 200:
                logger.info(f"✅ Logged metric to KB: {agent_name} {metric_type}={value}")
                return True
            else:
                logger.error(f"❌ Failed to log metric to KB: {response.text}")
                return False
    ...
    def run_agent_diagnostic(self, agent_name):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.info(f"✅ {agent_name} diagnostic successful ({response_time:.2f}s)")
                return {
                    "agent": agent_name,
                    "status": "success",
                ...
                logger.error(f"❌ {agent_name} diagnostic failed: {response.status_code}")
                return {
                    "agent": agent_name,
                    "status": "error",
                    "error": error_msg
            ...
            logger.error(f"❌ {agent_name} diagnostic error: {e}")
...
def main():
    ...
    if not integration.test_hakgal_connection():
        print("❌ Cannot connect to HAK/GAL API. Please ensure it's running on port 5002.")
        return

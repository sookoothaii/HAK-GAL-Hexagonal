...
// Implements human-in-the-loop learning system
...
const UnifiedQueryInterface: React.FC = () => {
  ...
  const reasonUrl = () => (isHex ? `${baseUrl}/api/reason` : `${baseUrl}/api/hrm/reason`);
  ...
  const logicalizeWithFallback = async (text: string): Promise<string[]> => {
    ...
    try {
      const r2 = await fetch(`http://localhost:5000/api/logicalize`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ text })
      });
      if (r2.ok) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!isHex) {
        const resp = await fetch(`${baseUrl}/api/command`, {
          method: 'POST', headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ command: 'ask', query: query.trim() })
        });
        if (resp.ok) { searchData = await resp.json(); searchOk = true; }
      }
      if (isHex || !searchOk) {
        // Hexagonal search fallback
        try {
...
Found Facts in Knowledge Base:
${extractedFacts.length > 0 ? extractedFacts.join('\n') : 'No existing facts found'}

Please provide:
1. A comprehensive explanation of the query
2. Analysis of the found facts (if any)
3. Suggestions for new facts that could be added to improve the knowledge base
4. Confidence assessment of the answer

Format your response to be educational and help the user understand both the answer and the reasoning process.
`;
      
      try {
        // Prefer Hex explanation proxy; fallback to 5000 explain/command
        ...
        try {
          ...
          if (proxyResp.ok) { llmData = await proxyResp.json(); ok = true; }
        } catch {}
        if (!ok) {
          // Original backend explain command
          const explainResp = await fetch(`${baseUrl}/api/command`, {
            method: 'POST', headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
              command: 'explain',
              query: query.trim(),
              context: {
                ...
                found_facts: extractedFacts,
                hrm_confidence: results.find(r => r.id === queryId)?.hrmConfidence || 0
              }
            })
          });
          if (explainResp.ok) { llmData = await explainResp.json(); ok = true; }
        }
        // Final fallback direct to 5000 proxy if baseUrl fails
        if (!ok) {
          const resp5000 = await fetch(`http://localhost:5000/api/llm/get-explanation`, {
            method: 'POST', headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ topic: query.trim() })
          });
          if (resp5000.ok) { llmData = await resp5000.json(); ok = true; }
        }
        ...
        if (ok && llmData) {
          console.log('LLM Explanation response:', llmData);
          let suggestedFacts = llmData.chatResponse?.suggested_facts || llmData.suggested_facts || parseSuggestedFacts(llmData, query.trim());

          // Normalize string[] to object[] and cap to 20
          if (Array.isArray(suggestedFacts)) {
            if (typeof suggestedFacts[0] === 'string') {
              suggestedFacts = (suggestedFacts as string[])
                .map((f: string) => (f.endsWith('.') ? f : f + '.'))
                .map((f: string) => ({ fact: f, confidence: 0.7, source: 'backend' }));
            }
            suggestedFacts = (suggestedFacts as any[]).slice(0, 20);
          }

          // If no suggestions, use backend logicalize on explanation + context
          if ((!suggestedFacts || suggestedFacts.length === 0)) {
            ...
            const facts = await logicalizeWithFallback(combined);
            if (facts && facts.length) {
              const canon = facts
                ...
                .map(f => ({ fact: f, confidence: 0.7, source: 'logicalize' }));
              if (canon.length) suggestedFacts = canon;
            }
          }
          // Calculate trust components based on all collected data
          ...
          const trustComponents: TrustComponents = {
            ...
            factualAccuracy: extractedFacts.length > 0 ? 0.8 : 0.3,  // Higher if facts found
            sourceQuality: extractedFacts.length > 0 ? (extractedFacts.length / 10) : 0.1,  // Based on fact count
            consensus: llmData.chatResponse ? 0.75 : 0.5,  // Higher if LLM responded
            humanVerified: false,
            ethicalAlignment: 0.7  // Default for now
          };
          
          // Update with full explanation and trust components
          setResults(prev => prev.map(r => 
            r.id === queryId 
              ? {
                  ...r,
  ...
  const parseSuggestedFacts = (llmData: any, originalQuery: string) => {
    const suggested = [];
    const text = llmData.chatResponse?.natural_language_explanation || 
                 llmData.response || '';
    
    // Look for logical statements in the response
    const factPatterns = text.match(/\w+\([^)]+\)/g) || [];
  ...
  const confirmFact = async (resultId: string, fact: string) => {
    ...
    try {
      ...
      if (isHex) {
        ...
        try {
          ...
          if (respHex.status === 201 || respHex.status === 200) ok = true;
            ...
            ok = false;
            toast.message('Already in knowledge base');
            // Entferne den Vorschlag aus der Liste, da er bereits existiert
            setResults(prev => prev.map(r => 
              r.id === resultId 
                ? { ...r, suggestedFacts: r.suggestedFacts?.filter(s => s.fact !== fact) }
                : r
            ));
          }
          else {
      ...
      if (!ok) {
        ...
        else if (response.status === 409) { ok = false; toast.message('Already in knowledge base'); }
        else {
      ...
      if (ok) {
        toast.success(`Fact added to knowledge base: ${fact}`);
        ...
        toast.error('Failed to add fact to knowledge base');
      }
    } catch (error) {
              ...
              <div className="max-w-md">
                ...
                <div className="grid grid-cols-2 gap-4 text-xs">
                  ...
                  <div className="flex flex-col items-center p-3 border rounded-lg">
                    ...
                    <span className="text-muted-foreground">2,576 facts</span>
                  </div>
                  <div className="flex flex-col items-center p-3 border rounded-lg">
                ...
                >
                  <Card>
                    ...
                    <CardContent className="pt-0">
                        ...
                        <div className="mb-4">
                          <TrustScoreCard
                            ...
                            sources={result.extractedFacts?.map((fact, idx) => ({
                              fact,
                              confidence: 0.8,  // Default confidence for existing facts
                              id: `fact-${idx}`
                            }))}
                            onVerify={() => {
                      ...
                      {/* Suggested Facts for Confirmation */}
                      {result.suggestedFacts && result.suggestedFacts.length > 0 && (
                        <Alert className="mb-4">
                          <Sparkles className="w-4 h-4" />
                          <AlertDescription>
                            <div className="flex items-center justify-between mb-2">
                              <span className="font-medium text-sm">Suggested Facts for Knowledge Base</span>
                              <Badge variant="outline" className="text-xs">
                              ...
                              {result.suggestedFacts.map((suggestion, idx) => (
                                <div key={idx} className="flex items-center justify-between p-2 bg-background rounded">
                                  <div className="flex-1">
                                    <code className="text-xs font-mono">{suggestion.fact}</code>
